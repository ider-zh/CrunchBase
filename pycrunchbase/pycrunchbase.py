...
class CrunchBase(object):
    ...
    Modified by ider    
    can use [node_data = self.get_node('entity', permalink)] get each entity
    return dict 
    you can use [json = JSONEncoder().encode(node_data)] to get a json

    to get more detail ,shuould import
    from resource import (
        Acquisition,
        FundingRound,
        Fund,
     ...
     use [entity = Entity(node_data)] to get entity object
     if items is more then 8 you should use 
     [data = cb.moredata(entity.past_team)]
    ...
    def moredata(self, page):
        ...
        if page.first_page_url:
            url_to_call = page.first_page_url
            return self._pagedata(page.name, url_to_call)
        elif page.next_page_url:
            url_to_call = page.next_page_url
            return self._pagedata(page.name, url_to_call)
        else:
